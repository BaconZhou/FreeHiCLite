...
The [`.hic`](https://github.com/aidenlab/juicer/wiki/Data) file is a highly compressed binary file, which is developed in the [Aiden Lab](http://aidenlab.org/). Which can be used in [juicebox](https://aidenlab.org/juicebox/) for contact matrix visulization.

The `.hic` file is formated as [HiCFormat](https://github.com/aidenlab/Juicebox/blob/master/HiCFormatV8.md). To program with `.hic` file, they provide [straw](https://github.com/aidenlab/straw) and [Dump](https://github.com/aidenlab/juicer/wiki/Data-Extraction) to extract the information from the `.hic` file. The `FreeHiCLite::readJuicer()` adopts most from the C++ version of straw.
...
The following figure shows how juicebox visualizied a human genome at chromosome 1.
...
To access basic information about a `.hic` file. We can use function `readJuicerInformation()`. For example, we want to know which *genomId* and what are the available *resolutions* of a remote file locates in: [https://hicfiles.s3.amazonaws.com/hiseq/gm12878/in-situ/combined.hic](). Besides, it will also extract the chromsomes sizes.
...
The 'FreeHiCLite' package provide a `readJuicer()` function to read file of chromsomes at a specific resolution.
...
- `contact`: A list of contact matrixs. For each matrix, the name stands for `chrA_chrB`, and matrix has has format (x, y, count).
...
java -jar /path/to/juicer_tool.jar pre test.txt test.hic genomId 
```
