...
float KDTreeResultVector::replace_maxpri_elt_return_new_maxpri(
    KDTreeResult &e) {
...
KDTree::KDTree(KDTreeArray &data_in, bool rearrange_in, int dim_in)
    : the_data(data_in), N(data_in.size()), dim(2), sort_results(false),
      rearrange(rearrange_in), root(NULL), data(NULL), ind(N) {
    //
...
class SearchRecord {
  ...
  public:
    SearchRecord(contactRecord &qv_in, KDTree &tree_in,
...
void KDTree::n_nearest_brute_force(contactRecord&qv, int nn,
...
void KDTree::n_nearest(contactRecord &qv, int nn,
                       KDTreeResultVector &result) {
...
void KDTree::r_nearest(contactRecord &qv, float r2,
                       KDTreeResultVector &result) {
...
int KDTree::r_count(contactRecord &qv, float r2) {
    // search for all within a ball of a certain radius
    {
        KDTreeResultVector result;
        SearchRecord sr(qv, *this, result);
...
void KDTreeNode::search(SearchRecord &sr) {
    ...
    if ((left == NULL) && (right == NULL)) {
        ...
        if ((nfarther != NULL) && (abs(extra) < sr.ballsize)) {
            // first cut
...
inline bool KDTreeNode::box_in_search_range(SearchRecord &sr) {
    ...
    for (int i = 0; i < dim; i++) {
        dis2 += abs(dis_from_bnd(sr.qv[i], box[i].lower, box[i].upper));
        if (dis2 > ballsize)
            return (false);
    }
    return (true);
...
void KDTreeNode::process_terminal_node(SearchRecord &sr) {
    ...
    for (int i = l; i <= u; i++) {
        ...
        if (rearrange) {
            ...
            dis = 0.0;
            for (int k = 0; k < dim; k++) {
                dis += abs(data[indexofi][k] - sr.qv[k]);
                if (dis > ballsize) {
                    early_exit = true;
                    break;
                }
        ...
        if (sr.result.size() < nn) {
            ...
            // Set the ball radius to the largest on the list (maximum
            // priority).
            if (debug) {
            ...
            //
            // if we get here then the current node, has a abs
            // distance smaller
